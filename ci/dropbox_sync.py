...
def dropbox_upload(local_path: str, remote_path: str, token: str, silent: bool = False):
    """Upload a single file to Dropbox. Uses chunked upload for files > 150 MB.

    Set silent=True to suppress per-file log output (used during bulk cache uploads).
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"  ✗ Upload failed after {max_retries} attempts: {local_path}")
...
def sync_cache_incremental_upload(cache_dir: str, base: str, token: str) -> tuple:
    ...
    for i, name in enumerate(to_upload, 1):
        ...
        if dropbox_upload(local_path, remote_path, token, silent=True):
            uploaded += 1
            consecutive_fails = 0
        else:
            failed += 1
            ...
            if consecutive_fails >= 10:
                print(f"  ⏳ 10 consecutive failures at file {i}/{len(to_upload)}, pausing 60s...")
                time.sleep(60)
                consecutive_fails = 0
        if i % 50 == 0 or i == len(to_upload):
            print(f"  ↑ {i}/{len(to_upload)} files uploaded...")
    
    skipped = len(local_files) - len(to_upload)
    fail_str = f", {failed} failed" if failed else ""
    print(f"  ✓ Cache upload complete: {uploaded} uploaded, {skipped} already in sync{fail_str}")
