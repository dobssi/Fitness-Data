...
| A007 | Johan | GAP | Polar watch + Stryd, newest athlete. Two-job workflow. |
...
│   ├── polar_ingest.py      # Polar watch FIT ingest
...
- **Onboard workflow generation broken:** `onboard_athlete.py` builds YAML from inline f-strings — produces broken single-job workflow. Needs refactor to use PaulTest workflow as template with placeholder substitution.
...
- Don't hardcode athlete-specific values in Python scripts
